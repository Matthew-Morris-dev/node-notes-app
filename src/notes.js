...
const removeNote = (title) => {
    ...
    if (notesToKeep.length !== notes.length) {
        ...
        if (notesToKeep.length > 0) {
            ...
            if (fs.existsSync(__dirname + "/data/notes.json")) {
                fs.unlinkSync(__dirname + "/data/notes.json");
            }
        }
    } else {
...
const saveNotes = (notes) => {
    ...
    fs.writeFileSync(__dirname + "/data/notes.json", dataJSON);
};

const loadNotes = () => {
    try {
        const dataBuffer = fs.readFileSync(__dirname + "/data/notes.json");
